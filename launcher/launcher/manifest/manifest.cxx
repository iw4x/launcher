...
namespace launcher
{
  ...
  manifest::
  manifest (const std::string& json_str, manifest_format k)
    : kind (k)
  {
    try
    {
    ...
    {
      throw std::runtime_error (std::string ("failed to parse manifest: ") + e.what ());
    }
  }

  manifest::
  manifest (const json::value& jv, manifest_format k)
    : kind (k)
  {
    try
    {
  ...
  validate () const
  {
    // In absence of manifest_traits, integrity checks can be
    // evaluated generically directly on elements. Currently returning true.
    return true;
  }
    ...
    if (obj.contains ("archives") && obj.at ("archives").is_array ())
    {
      for (const auto& ja : obj.at ("archives").as_array ())
      {
        ...
        if (ao.contains ("blake3") && ao.at ("blake3").is_string ())
          archive.hash = launcher::hash (json::value_to<std::string> (ao.at ("blake3")));
        ...
        if (ao.contains ("name") && ao.at ("name").is_string ())
          archive.name = json::value_to<std::string> (ao.at ("name"));

        if (ao.contains ("url") && ao.at ("url").is_string ())
          archive.url = json::value_to<std::string> (ao.at ("url"));
    ...
    if (obj.contains ("files") && obj.at ("files").is_array ())
    {
      for (const auto& jf : obj.at ("files").as_array ())
      {
        ...
        if (fo.contains ("blake3") && fo.at ("blake3").is_string ())
          file.hash = launcher::hash (json::value_to<std::string> (fo.at ("blake3")));
        ...
        if (fo.contains ("path") && fo.at ("path").is_string ())
          file.path = json::value_to<std::string> (fo.at ("path"));

        if (fo.contains ("asset_name") && fo.at ("asset_name").is_string ())
          file.asset_name = json::value_to<std::string> (fo.at ("asset_name"));

        if (fo.contains ("archive") && fo.at ("archive").is_string ())
          file.archive_name = json::value_to<std::string> (fo.at ("archive"));
  ...
  asio::awaitable<manifest> manifest::
  parse_async (const std::string& json_str, manifest_format k)
  {
